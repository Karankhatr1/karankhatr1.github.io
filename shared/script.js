/* ==========================================================================
   Site logic — shared by every view. You normally don't touch this file.
   • Who you are lives in            shared/profile.js
   • What each audience sees lives in that view's own  data.js
   ========================================================================== */

(function () {
  "use strict";

  /* Merge shared profile + this view's content into one SITE object.
     Anything set in a view's data.js overrides the shared profile. */
  ...
  function renderCaseStudyInto(root, cs, opts) {
    ...
    /* prev / next — built only from THIS view's own case-study list,
       so it can never link across to the other view */
  ...
  var res = $("#resumeBtn");
  if (res) {
    if (SITE.resume) { res.href = SITE.resume; res.target = "_blank"; res.rel = "noopener noreferrer"; }
    else res.remove();
  }
